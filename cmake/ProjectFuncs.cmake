function(custengine_setup_target target)
    target_compile_definitions(${target} PRIVATE "$<$<CONFIG:DEBUG>:CUSTENGINE_DEBUG>")
    ...
    if("${CMAKE_BUILD_TYPE}" STREQUAL "Release")
        ...
        if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "Clang")
            ...
            target_link_options(${target} PRIVATE -fuse-ld=${CUSTENGINE_LINKER} -flto=thin)
        else()
            set(_CORE_COMPILE_ARGS ${_CORE_COMPILE_ARGS} -flto)
            set(_CORE_LINKER_ARGS ${_CORE_LINKER_ARGS} -flto)
            target_link_options(${target} PRIVATE -fuse-ld=${CUSTENGINE_LINKER} -flto)
        endif()

    endif()

    if("asan" IN_LIST CUSTENGINE_BUILD_FEATURES)
        # Error if someone's trying to mix asan and tsan together,
        # they aren't compatible.
        if("tsan" IN_LIST CUSTENGINE_BUILD_FEATURES)
            message(FATAL_ERROR "ASAN and TSAN cannot be used together.")
        endif()

        message(STATUS "Enabling ASAN for target ${target} because it was in CUSTENGINE_BUILD_FEATURES")
        set(_CORE_COMPILE_ARGS ${_CORE_COMPILE_ARGS} -fsanitize=address)
        set(_CORE_LINKER_ARGS ${_CORE_LINKER_ARGS} -fsanitize=address)
    endif()

    if("tsan" IN_LIST CUSTENGINE_BUILD_FEATURES)
        message(STATUS "Enabling TSAN for target ${target} because it was in CUSTENGINE_BUILD_FEATURES")
        set(_CORE_COMPILE_ARGS ${_CORE_COMPILE_ARGS} -fsanitize=thread)
        set(_CORE_LINKER_ARGS ${_CORE_LINKER_ARGS} -fsanitize=thread)
    endif()

    if("ubsan" IN_LIST CUSTENGINE_BUILD_FEATURES)
        message(STATUS "Enabling UBSAN for target ${target} because it was in CUSTENGINE_BUILD_FEATURES")
...
function(custengine_set_alternate_linker)
    find_program(LINKER_EXECUTABLE ld.${CUSTENGINE_LINKER} ${CUSTENGINE_LINKER})
    if(LINKER_EXECUTABLE)
        message(STATUS "Using ${CUSTENGINE_LINKER} as linker for custengine projects")
    else()
        message(FATAL_ERROR "Linker ${CUSTENGINE_LINKER} does not exist on your system. Please specify one which does or omit this option from your configure command.")
    endif()
endfunction()
