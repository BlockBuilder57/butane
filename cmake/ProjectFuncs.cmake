function(butane_setup_target target)
    target_compile_definitions(${target} PRIVATE "$<$<CONFIG:DEBUG>:BUTANE_DEBUG>")
    ...
    if("${CMAKE_BUILD_TYPE}" STREQUAL "Release")
        ...
        if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "Clang")
            ...
            target_link_options(${target} PRIVATE -fuse-ld=${BUTANE_LINKER} -flto=thin)
        else()
            set(_CORE_COMPILE_ARGS ${_CORE_COMPILE_ARGS} -flto)
            set(_CORE_LINKER_ARGS ${_CORE_LINKER_ARGS} -flto)
            target_link_options(${target} PRIVATE -fuse-ld=${BUTANE_LINKER} -flto)
        endif()

    endif()

    if("asan" IN_LIST BUTANE_BUILD_FEATURES)
        # Error if someone's trying to mix asan and tsan together,
        # they aren't compatible.
        if("tsan" IN_LIST BUTANE_BUILD_FEATURES)
            message(FATAL_ERROR "ASAN and TSAN cannot be used together.")
        endif()

        message(STATUS "Enabling ASAN for target ${target} because it was in BUTANE_BUILD_FEATURES")
        set(_CORE_COMPILE_ARGS ${_CORE_COMPILE_ARGS} -fsanitize=address)
        set(_CORE_LINKER_ARGS ${_CORE_LINKER_ARGS} -fsanitize=address)
    endif()

    if("tsan" IN_LIST BUTANE_BUILD_FEATURES)
        message(STATUS "Enabling TSAN for target ${target} because it was in BUTANE_BUILD_FEATURES")
        set(_CORE_COMPILE_ARGS ${_CORE_COMPILE_ARGS} -fsanitize=thread)
        set(_CORE_LINKER_ARGS ${_CORE_LINKER_ARGS} -fsanitize=thread)
    endif()

    if("ubsan" IN_LIST BUTANE_BUILD_FEATURES)
        message(STATUS "Enabling UBSAN for target ${target} because it was in BUTANE_BUILD_FEATURES")
...
function(butane_set_alternate_linker)
    find_program(LINKER_EXECUTABLE ld.${BUTANE_LINKER} ${BUTANE_LINKER})
    if(LINKER_EXECUTABLE)
        message(STATUS "Using ${BUTANE_LINKER} as linker for Butane projects")
    else()
        message(FATAL_ERROR "Linker ${BUTANE_LINKER} does not exist on your system. Please specify one which does or omit this option from your configure command.")
    endif()
endfunction()
